...
acsitst
	...
	; Local variables
		rsreset
acsitst.loops	rs.l	1               ; Loop counter

	link	#-4,a6                  ; Create a stack frame
...
.chkvrs	; Do an Inquiry and check the hardware version
...
.tstcmd	print	msg.tstcmd(pc)
	...
	lea	acsitst.loops(a6),a0    ; Initialize loop counter
...
.ok	lea	acsitst.loops(a6),a0    ; Decrement test loop counter
	sub.w	#1,(a0)                 ;
	bne.b	.next                   ; Loop tests until successful

.tstfcm	lea	acsitst.loops(a6),a0    ; Initialize loop counter
...
.tstzcm	lea	acsitst.loops(a6),a0    ; Initialize loop counter
	move.w	#16,(a0)                ; Do 16 test loops
...
.tstzwc	lea	acsitst.loops(a6),a0    ; Initialize loop counter
	move.w	#64,(a0)                ; Do 64 test loops
...
.halfcm	; The hardest test: send a 1 byte command, then chain with a valid one !
...
.succss	print	msg.success(pc)         ; Display "Test successful"
	bra.w	again                   ; Go back to device selection

.filbuf	lea	bss+buf(pc),a0
	bsr.b	setrpt
.copy	move.l	d0,(a0)+
	dbra	d1,.copy
	rts
...
.exccmd	; Call acsicmd and check for a timeout
